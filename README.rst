...
DJANGO_SERVER_EMAIL                     SERVER_EMAIL                n/a                                            "overdewey <noreply@overdewey.com>" 
DJANGO_EMAIL_SUBJECT_PREFIX             EMAIL_SUBJECT_PREFIX        n/a                                            "[overdewey] "
======================================= =========================== ============================================== ======================================================================
...
.. code-block:: bash
    ...
    cd overdewey
    celery -A overdewey.taskapp worker -l info
...
To run the tests, enter the overdewey/tests directory and run the following commands::
...
..  code-block:: bash
    ...
    git remote add dokku dokku@yourservername.com:overdewey
    git push dokku master
    ssh -t dokku@yourservername.com dokku redis:create overdewey-redis
    ssh -t dokku@yourservername.com dokku redis:link overdewey-redis overdewey
    ssh -t dokku@yourservername.com dokku postgres:create overdewey-postgres
    ssh -t dokku@yourservername.com dokku postgres:link overdewey-postgres overdewey
    ssh -t dokku@yourservername.com dokku config:set overdewey DJANGO_SECRET_KEY=RANDOM_SECRET_KEY_HERE
    ssh -t dokku@yourservername.com dokku config:set overdewey DJANGO_SETTINGS_MODULE='config.settings.production'
    ssh -t dokku@yourservername.com dokku config:set overdewey DJANGO_AWS_ACCESS_KEY_ID=YOUR_AWS_ID_HERE
    ssh -t dokku@yourservername.com dokku config:set overdewey DJANGO_AWS_SECRET_ACCESS_KEY=YOUR_AWS_SECRET_ACCESS_KEY_HERE
    ssh -t dokku@yourservername.com dokku config:set overdewey DJANGO_AWS_STORAGE_BUCKET_NAME=YOUR_AWS_S3_BUCKET_NAME_HERE
    ssh -t dokku@yourservername.com dokku config:set overdewey DJANGO_MAILGUN_API_KEY=YOUR_MAILGUN_API_KEY
    ssh -t dokku@yourservername.com dokku config:set overdewey DJANGO_MAILGUN_SERVER_NAME=YOUR_MAILGUN_SERVER
    ssh -t dokku@yourservername.com dokku run overdewey python manage.py migrate
    ssh -t dokku@yourservername.com dokku run overdewey python manage.py createsuperuser
...
application stacks on one host, remove the port setting entirely and add `VIRTUAL_HOST=overdewey.com` to your env file.
This pass all incoming requests on `nginx-proxy` to the nginx service your application is using.

.. _nginx-proxy: https://github.com/jwilder/nginx-proxy

Postgres is saving its database files to `/data/overdewey/postgres` by default. Change that if you wan't
...
Place it in `/etc/supervisor/conf.d/overdewey.conf` and run::

    supervisorctl reread
    supervisorctl start overdewey
